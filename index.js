...
if (!publicVapidKey || !privateVapidKey) {
  console.error("VAPID keys are not set in the .env file. Please ensure VAPID_PUBLIC_KEY and VAPID_PRIVATE_KEY are present.");
} else {
  webpush.setVapidDetails(
    "mailto:test@example.com", // Replace with your email address
    publicVapidKey,
    privateVapidKey
  );
  console.log("Web Push has been configured.");
}
const app = express();
  ...
  try {
    ...
    app.post("/job-requests", async (req, res) => {
      try {
        ...
        try {
          const city = newJob.city;
          const jobTitle = newJob.title;

          // Find users in the same city who are subscribed to notifications
          ...
          if (usersToNotify.length > 0) {
            console.log(`Found ${usersToNotify.length} users in ${city} to notify.`);
              ...
              return webpush.sendNotification(user.pushSubscription, payload)
                .catch(error => {
                  ...
                  if (error.statusCode === 410) {
                    console.log(`Subscription for user ${user.uid} has expired. Removing.`);
                    return usersCollection.updateOne({ _id: user._id }, { $unset: { pushSubscription: "" } });
                  } else {
                    console.error(`Error sending notification to user ${user.uid}:`, error.body);
                  }
                });
            });
          ...
          console.error("An error occurred while sending push notifications:", notificationError);
        }
    ...
    app.post("/api/save-subscription", verifyToken, async (req, res) => {
